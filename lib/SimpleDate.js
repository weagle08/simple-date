 ...
 * @returns {number} - the days difference, ie: the number of days you can add to this date to arrive at the test date
 */
...
SimpleDate.prototype.toString = function(format){
    ...
    if(dayFrmt == 'dd' && dt.toString().length === 1) {
        dt = '0' + dt.toString();
    }
...
SimpleDate._calculateOneTimePeriod = function(start, end) {
    return [new SimpleDate(start.getYear(), start.getMonth(), start.getDate())];
};

SimpleDate._calculateDailyPeriod = function(start, end) {
...
SimpleDate._calculateWeeklyPeriod = function(start, end) {
    var diff = start.getDaysDifference(end);
...
SimpleDate._calculateBiWeeklyPeriod = function(start, end) {
    var diff = start.getDaysDifference(end);
...
SimpleDate._calculateMonthlyPeriod = function(start, end) {
...
SimpleDate.calculatePeriodDates = function(start, end, frequency) {
    ...
    if(frequency === 0){
        return SimpleDate._calculateOneTimePeriod(startDate, endDate);
    } else if(frequency === 1) {
        return SimpleDate._calculateDailyPeriod(startDate, endDate);
    } else if(frequency === 2) {
        return SimpleDate._calculateWeeklyPeriod(startDate, endDate);
    } else if(frequency === 3) {
        return SimpleDate._calculateBiWeeklyPeriod(startDate, endDate);
    } else if(frequency === 4) {
        return SimpleDate._calculateMonthlyPeriod(startDate, endDate);
    }
